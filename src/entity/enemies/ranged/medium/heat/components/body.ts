...
export class HeatEnemyBody extends EnemyBody {
  public heatEnemy: HeatEnemy;
  public constructor(public enemy: HeatEnemy) {
    super(enemy);
    this.heatEnemy = enemy;
  }

  start() {
    const heatEnemy = this.heatEnemy;
    const scene = this.heatEnemy.scene;
    const worldScale = adapter.planckPhysics.worldScale;

    const bodyRadius = 25;
    this.body = adapter.createCircle(heatEnemy.x, heatEnemy.y, bodyRadius, {
      filterGroupIndex: -1,
    });
    ...
    // this.heat.container.addAt(
    //   scene.add.rectangle(0, -25, 50, 50, 0xff0000),
    //   0
    // );

    const boydTagName = "heat-enemy-body";
  ...
  update() {
    ...
    this.heatEnemy.x = this.heatEnemy.container.x = Math.floor(screenPosition.x);
    this.heatEnemy.y = this.heatEnemy.container.y = Math.floor(screenPosition.y);
