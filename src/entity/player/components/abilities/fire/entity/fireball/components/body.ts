...
export class FireballBody extends EntityComponent {
  ...
  public constructor(public parent: Fireball) {
    super();
  }

  public start() {
    this.body = adapter.createCircle(this.parent.x, this.parent.y, 30);
    this.body.setGravityScale(0.4);
    this.body.getFixtureList().setSensor(true);

    this.body.setUserData({ tag: "fireball", parent: this });
  ...
  private _registerContacts() {
    ...
    this.contactsManager.set(
      "enemy",
      new BaseContact(this, enemiesTag, "fireball")
    );
    this.contactsManager.set(
      "ground",
      new BaseContact(this, ["ground"], "fireball")
    );
  ...
  public beginContact(contact: Contact) {
    ...
    if (bodiesTag.has("fireball") && bodiesTag.has("ground")) {
      const body = bodiesTag.get("fireball");
    ...
    if (enemyContacts) {
      ...
      contacts.forEach((contact) => {
        ...
        if (!movement.knockBack && enemy.landed) {
          ...
          movement.enemy.life -= 2 * 1.5 * s.count;
          this.destroyed = true;
        }
      });
    }
